...
class DocBlockGenerator {
    ...
    static generateMethodDocBlock(doc) {
        ...
        if (config.get('authorTag')){
            docBlock += indent + ` * @author     ${config.get('authorTag')} \r\n`;
        }

        if (config.get('authorSeeUrl')){
            docBlock += indent + ` * @see       {@link ${config.get('authorSeeUrl')}} \r\n`;
        }
        
        if (config.get('companySeeUrl')){
            docBlock += indent + ` * @see       {@link ${config.get('companySeeUrl')}} \r\n`;
        }

        if (config.get('copyrightCompanyName')){
            docBlock += indent + ` * @copyright ${config.get('copyrightCompanyName')} \r\n`;
        }
